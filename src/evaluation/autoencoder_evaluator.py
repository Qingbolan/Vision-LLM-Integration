...
def evaluate_autoencoder(model, dataloader, device, checkpoint_path='./checkpoints/', num_images=10):
    model.eval()
    total_loss = 0.0
    mse = nn.MSELoss()
    original_images = []
    ...
    print(f'Average Reconstruction Loss (MSE): {avg_loss:.4f}')
    ...
    plt.axis('off')
    plt.savefig(os.path.join(checkpoint_path, 'dcae_reconstruction.png'))
    plt.close()
    print(f'Reconstruction images saved at {os.path.join(checkpoint_path, "dcae_reconstruction.png")}')